import { isUndef, forEach } from '../../util';
...
function flush(value) {
    // run callback
    forEach(DEVICE_ID_CALLBACK_QUEUE, function (callback) {
        callback(value);
    });
    // clear queue
...
function getSuid(callback) {
    if (!isUndef(GLOBAL.KDJSBridge2)) {
        GLOBAL.KDJSBridge2.call('WDJSBridge', 'getSuid', {}, function (res) {
...
function getCuid(callback) {
    if (!isUndef(GLOBAL.KDJSBridge2)) {
        GLOBAL.KDJSBridge2.call('WDJSBridge', 'getCuid', {}, function (res) {
