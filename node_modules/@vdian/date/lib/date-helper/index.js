import { TOKEN_REGEX_MAP, REGEX_SPECIAL_CHARACTER, PARSE_TOKEN, FORMAT_TOKEN, DEFAULT_FORMAT, DEFAULT_YEAR, DEFAULT_MONTH, DEFAULT_DATE, DEFAULT_HOUR, DEFAULT_MINUTE, DEFAULT_SECOND } from '../constants';
...
function regexEscape(regText) {
    return regText.replace(REGEX_SPECIAL_CHARACTER, '\\$&');
}
// 把年份最后两位变成0
...
var getParseInfo = function (input, format) {
    ...
    // 记录格式化符号在捕获组中的索引
    regexpText = regexpText.replace(PARSE_TOKEN, function (token) {
        result.tokenMap[token] = index++;
        return TOKEN_REGEX_MAP[token];
    });
...
var DateHelper = /** @class */ (function () {
    ...
    DateHelper.prototype.format = function (date, format) {
        ...
        format = format.replace(FORMAT_TOKEN, function (part) { return getValueByPattern(part, date) + ''; });
        return format;
    };
    /**
     * 解析时间
    ...
    DateHelper.prototype.parse = function (input, format) {
        ...
        var year = parts[fmt.yyyy] || parts[fmt.yy] || DEFAULT_YEAR;
        if (matchParts[fmt.yy]) {
            year += get2Year(new Date());
        }
        var month = (parts[fmt.MM] || parts[fmt.M] || DEFAULT_MONTH) - 1;
        var date = parts[fmt.dd] || parts[fmt.d] || DEFAULT_DATE;
        var hour = parts[fmt.HH] || parts[fmt.H] || DEFAULT_HOUR;
        var minute = parts[fmt.mm] || parts[fmt.m] || DEFAULT_MINUTE;
        var second = parts[fmt.ss] || parts[fmt.s] || DEFAULT_SECOND;
